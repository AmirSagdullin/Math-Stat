...
for i in range(len(data_before)):
    if data_before[i] > data_after[i]:
        ...
        binary_data.append(0) # Эффекта нет (увеличилось или не изменилось)

# Частота появления благоприятных событий
frequency = sum(binary_data)/len(binary_data)
print(f"Частота ожидаемого эффекта (уменьшение) = {frequency}, {sum(binary_data)} из {len(binary_data)}")
...
# Вычисляем критическое значение для 1%-ой критической области
critical_value = binom.ppf(1 - alpha, len(binary_data), 0.5) # правая граница
print("1%-ая критическая область:", critical_value)
